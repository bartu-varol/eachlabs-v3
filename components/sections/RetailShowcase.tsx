...
import { RetailExamples } from './RetailExamples';
import { RetailProblems } from './RetailProblems';
import { RetailAnatomy } from './RetailAnatomy';

/* ──────────────────────────────────────────────────────────────────────────
   RetailShowcase — bespoke /usecases/retail page.

     1. Hero            — split: copy + animated asset-pipeline demo
     2. Stats           — 4 quantitative anchors (savings, speed, locales)
     3. Examples        — 6 commerce-pipeline scenarios (model, try-on, bg, ...)
     4. Problems        — 6 enterprise pains × the each::xxx that fixes each
     5. Anatomy         — "1 brief → 12 markets" parallel fan-out
     6. Why it fits     — 4 platform tenets specific to retail/commerce
     7. Story           — metric-driven customer quote
     8. CTA
────────────────────────────────────────────────────────────────────────── */

export function RetailShowcase({ uc }: { uc: UseCaseDef }) {
  return (
    <>
      {/* 1. HERO */}
      <section className="container py-16 md:py-24">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,560px)] gap-10 lg:gap-16 items-start mt-8">
          ...
          >
            ...
            {/* WHAT / WHY / HOW for retail */}
            <ul className="mt-8 flex flex-col gap-2.5 max-w-[540px]">
              <BulletLine
                accent="WHAT"
                text="A studio replacement: product photo, on-model, lifestyle, video, and 12 locales — all from one each.run()."
              />
              <BulletLine
                accent="WHY"
                text="Brand-safe gates by default. Audit per asset. Provider price = your price. Compliance reads the trace."
              />
              <BulletLine
                accent="HOW"
                text={
                  <>
                    Define a campaign workflow once with your{' '}
                    <span className="font-mono text-spark">brand_voice</span>{' '}
                    profile. Trigger per launch. Read the audit trail.
                  </>
                }
              />
            </ul>
      ...
      <section className="container border-t border-rule py-24 md:py-32">
        <div className="max-w-[680px] mx-auto text-center">
          <h2 className="font-display font-semibold text-[34px] md:text-[48px] leading-[1.05] tracking-tightest text-ink">
            One brief. Twelve markets. No studio.
          </h2>
          <p className="text-ink2 text-[15px] mt-6">
            Replace the booking calendar with a workflow. Replace the spreadsheet
            with a trace. Replace the agency markup with provider-price billing.
          </p>
