    ...
    <section className="relative border-t border-rule overflow-hidden">
      ...
      <div className="container py-24 md:py-28 relative">
        ...
        <div className="mt-8 bg-surface border border-rule2 rounded-md overflow-hidden">
          {/* Header row */}
          <div className="grid grid-cols-[1fr_2fr_70px_60px_70px_90px_90px] gap-3 md:gap-4 px-4 md:px-5 py-3 border-b border-rule2 bg-bg/40 font-mono text-[10px] uppercase tracking-eyebrow text-ink3">
            <span>provider</span>
            <span>model</span>
            <span>quality</span>
            <span>type</span>
            ...
            <span className="text-right">runs at ${budget}</span>
          </div>
              ...
              filtered.map((m, i) => {
                ...
                return (
                  <motion.div
                    ...
                    className="grid grid-cols-[1fr_2fr_70px_60px_70px_90px_90px] gap-3 md:gap-4 px-4 md:px-5 py-3 border-b border-rule/60 last:border-b-0 hover:bg-bg/40 font-mono text-[12.5px] items-center"
                  >
                    ...
                    <span className="text-right tabular-nums" style={{ color: accent }}>
                      {formatPrice(m.price)}
                    </span>
        ...
        <p className="mt-4 font-mono text-[10px] text-ink3 text-center">
          {filtered.length} of {MODEL_PRICES.filter((p) => p.modality === modality).length} {modality.toLowerCase()} models · prices may vary by input
        </p>
      </div>
    </section>
  );
