...
import { MarketingExamples } from './MarketingExamples';
import { MarketingProblems } from './MarketingProblems';
import { MarketingAnatomy } from './MarketingAnatomy';

export function MarketingShowcase({ uc }: { uc: UseCaseDef }) {
  return (
    <>
      {/* HERO */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,560px)] gap-10 lg:gap-16 items-start mt-8">
          ...
          >
            ...
            <ul className="mt-8 flex flex-col gap-2.5 max-w-[540px]">
              <BulletLine accent="WHAT" text="A brand voice profile that becomes every channel asset — IG, TikTok, web, OOH, video — without drift." />
              <BulletLine accent="WHY"  text="Brand teams maintain ONE thing. The platform applies it to N surfaces, N seasons, N partners." />
              <BulletLine accent="HOW"
                text={
                  <>
                    Compile a profile once. Pass{' '}
                    <code className="font-mono text-[12.5px] text-spark">brand_voice</code>{' '}
                    to each.run(). Read the trace.
                  </>
                }
              />
            </ul>
      ...
      <section className="container border-t border-rule py-24 md:py-32">
        <div className="max-w-[680px] mx-auto text-center">
          <h2 className="font-display font-semibold text-[34px] md:text-[48px] leading-[1.05] tracking-tightest text-ink">
            One profile. Every surface. Same brand.
          </h2>
          <p className="text-ink2 text-[15px] mt-6">
            Replace the agency markup with provider-price billing. Replace the
            review queue with the trace. Replace brand drift with a one-line
            profile.
          </p>
          <div className="flex flex-wrap gap-3 justify-center mt-10">
            <Button href="/signup" variant="primary">Start free →</Button>
            <Button href="/contact" variant="secondary">Talk to brand</Button>
          </div>
        </div>
      </section>
    </>
