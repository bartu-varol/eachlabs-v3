...
import { GamingDemo } from './GamingDemo';
import { GamingExamples } from './GamingExamples';
import { GamingProblems } from './GamingProblems';
import { GamingAnatomy } from './GamingAnatomy';

export function GamingShowcase({ uc }: { uc: UseCaseDef }) {
  return (
    <>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,560px)] gap-10 lg:gap-16 items-start mt-8">
          ...
          >
            ...
            <ul className="mt-8 flex flex-col gap-2.5 max-w-[540px]">
              <BulletLine accent="WHAT" text="Live-event content at the speed of live-service: NPCs, dialogue, textures, music, voice — one workflow per drop." />
              <BulletLine accent="WHY"  text="Workflow versioning means rollback is a config flip. On-prem option keeps sensitive IP inside your VPC." />
              <BulletLine accent="HOW"
                text={
                  <>
                    Pin a workflow version per event. Set{' '}
                    <code className="font-mono text-[12.5px] text-spark">retention</code>.
                    Roll back in one string when needed.
                  </>
                }
              />
            </ul>
      ...
      <section className="container border-t border-rule py-24 md:py-32">
        <div className="max-w-[680px] mx-auto text-center">
          <h2 className="font-display font-semibold text-[34px] md:text-[48px] leading-[1.05] tracking-tightest text-ink">
            Ship the event. Pin the version. Sleep at launch.
          </h2>
          <p className="text-ink2 text-[15px] mt-6">
            One SDK behind every asset category. Rollback is a string change.
            VPC option for sensitive IP. Audit on every line of dialogue, every
            texture variant, every music cue.
          </p>
          <div className="flex flex-wrap gap-3 justify-center mt-10">
            <Button href="/signup" variant="primary">Start free →</Button>
            <Button href="/contact" variant="secondary">Talk to live-ops</Button>
          </div>
        </div>
      </section>
    </>
