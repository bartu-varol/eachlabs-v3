...
/* ── Geometry — viewBox is 580×340, nodes positioned by center coords ───── */
...
const NODES: Record<NodeId, { x: number; y: number; label: string; sub?: string; tone?: 'spark' | 'highlight' | 'sun' }> = {
  input:   { x:  50, y: 170, label: 'INPUT' },
  enhance: { x: 165, y: 170, label: 'enhance', sub: 'gpt-4o', tone: 'highlight' },
  image:   { x: 320, y:  60, label: 'image',   sub: 'flux-2',     tone: 'spark' },
  video:   { x: 320, y: 170, label: 'video',   sub: 'kling-v3',   tone: 'spark' },
  audio:   { x: 320, y: 280, label: 'audio',   sub: 'eleven-v3',  tone: 'sun' },
  merge:   { x: 450, y: 170, label: 'merge',   sub: 'compose',    tone: 'highlight' },
  output:  { x: 540, y: 170, label: 'OUT' },
};
...
function curve(from: NodeId, to: NodeId): string {
  const a = NODES[from];
  const b = NODES[to];
  // Smooth horizontal bezier: control points pull the curve sideways.
      ...
      <AnimatePresence>
        {phase === 'parallel' && (
          <motion.div
            ...
            className="absolute font-mono text-[9px] uppercase tracking-eyebrow text-spark"
            style={{
              left: `${(NODES.image.x / VIEW_W) * 100}%`,
              top: 0,
